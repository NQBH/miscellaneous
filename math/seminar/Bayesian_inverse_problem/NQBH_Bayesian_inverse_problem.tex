...
\begin{equation*}
    \mbox{Posterior}\to\mbox{Forward Model } G\to\left\{\begin{split}
        &\to\mbox{(Noisy) Data } y\to\mbox{Posterior},\\
        &\to\mbox{Quantity of interest}. 
    \end{split}\right.
\end{equation*}
\begin{itemize}
...
Find the unknown data $u\in\mathbb{R}^n$ from noisy observations
\begin{align*}
    y = \mathcal{G}(u) + \eta,\ \eta\in\mathbb{R}^K.
\end{align*}
\begin{itemize}
    \item $u\in\mathbb{R}^n$ random variable with Lebesgue density $\rho_0(u)$.
    \item $\mathcal{G}:\mathbb{R}^n\to\mathbb{R}^K$ measurable function.
    \item $\eta\in\mathbb{R}^K$ independent of $u$ ($u\bot\eta$), distributed according to measure $\mu_\eta$ with Lebesgue density $\rho(\eta)$.
    \item $y|u$ is then distributed according to measure $\mu_u$ with Lebesgue density $\rho(y - \mathcal{G}(u))$.???
    \item $(u,y)\in\mathbb{R}^n\times\mathbb{R}^K$ is a random variable with Lebesgue density $\rho(y - \mathcal{G}(u))\rho_0(u)$.
\end{itemize}

\begin{theorem}[Bayes' Theorem]
    Assume that
    \begin{align*}
        Z\coloneqq\int_{\mathbb{R}^n} \rho(y - \mathcal{G}(u))\rho_0(u){\rm d}u > 0.
    \end{align*}
...
\begin{itemize}
    \item $\rho_0(u)$ is the \textit{prior density}.
    \item $\rho(y - \mathcal{G}(u))$ is the \textit{likelihood}.
    \item $\rho^y(u)$ is the posterior density.
    \item $\Phi(u;y) = -\log \rho(y - G(u))$ is called the \textit{potential} (negative log likelihood).
    \item Bayes' formula expresses
    \begin{align*}
        \mathbb{P}(u|y) = \frac{1}{\mathbb{P}(y)}\mathbb{P}(y|u)\mathbb{P}(u).
    \end{align*}
    \item Let $\mu^y$ be a measure on $\mathbb{R}^n$ with density $\rho^y$ and $\mu_0$ a measure on $\mathbb{R}^n$ with Lebesgue density $\rho_0$.
    
    Then, Bayes' theorem may be written as
    \begin{align*}
        \frac{{\rm d}\mu^y}{{\rm d}\mu_0}(u) = \frac{1}{Z}\exp\left(-\Phi(u;y)\right),\ Z = \int_{\mathbb{R}^n} \exp(-\Phi(u;y))\mu_0({\rm d}u).
    \end{align*}
    \item The expression for the Radon-Nikodym derivative is to be interpreted as follows: for all measurable function $f:\mathbb{R}^n\to\mathbb{R}$
    \begin{align*}
        \mathbb{E}_{\mu^y}f(u) = \mathbb{E}_{\mu_0}\left(\frac{{\rm d}\mu^y}{{\rm d}\mu_0}(u)f(u)\right).
    \end{align*}
\end{itemize}
...
\begin{example}[Comparison of CM \& MAP (1D)]
    ...
    \begin{equation*}
        ...
        \end{split}\right.
    \end{equation*}
    \textsf{Fig. Posterior density with parameter values $\alpha = \frac{1}{2}$, $\sigma_0 = 0.08$, $\sigma_1 = 0.04$.}
    
    \textsf{Fig. Posterior density with parameter values $\alpha = 0.01$, $\sigma_0 = 0.001$, $\sigma_1 = 0.1$.}
\end{example}
